...
it('supports cluster resources across the Kubernetes lifecycle', function (): void {
    ...
    $node = $a->execute(['team_id' => 'team-1', 'kind' => 'node', 'cluster_id' => 'cluster-1', 'name' => 'worker-1', 'status' => 'Ready', 'schedulable' => true]);
    $namespace = $a->execute(['team_id' => 'team-1', 'kind' => 'namespace', 'cluster_id' => 'cluster-1', 'name' => 'production']);
    ...
    $view = $a->execute(['team_id' => 'team-1', 'kind' => 'cluster-view', 'name' => 'all', 'cluster_ids' => ['cluster-1']]);
    expect($node->isSchedulable())->toBeTrue()->and($namespace->name)->toBe('production')->and($rbac->role)->toBe('admin')->and($workload->image)->toBe('nginx')->and($ingress->host)->toBe('example.test')->and($helm->chart)->toBe('app-chart')->and($storage->access_modes)->toContain('ReadWriteOnce')->and($autoscaler->max_replicas)->toBe(5)->and($upgrade->to_version)->toBe('1.29')->and($view->cluster_ids)->toContain('cluster-1');
});
