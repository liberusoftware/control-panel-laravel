...
final class MailAccountController
{
    ...
    public function operation(Request $request, RecordMailOperation $record): JsonResponse
    {
        ...
        $item=$record->execute(array_merge($data,['team_id'=>$teamId])); return response()->json(['data'=>['id'=>$item->getKey(),'type'=>'control-panel-mail-operation','attributes'=>$item->only(['mail_account_id','operation','status','details'])]],201);
    }

    public function alias(Request $request, CreateMailAlias $create): JsonResponse
    {
        $teamId=$request->user()?->current_team_id; abort_if($teamId===null,403,'A current team is required.'); $data=$request->validate(['domain'=>['required','string','max:255'],'address'=>['required','string','max:255'],'destinations'=>['required','array','min:1'],'destinations.*'=>['email']]); $item=$create->execute(array_merge($data,['team_id'=>$teamId])); return response()->json(['data'=>['id'=>$item->getKey(),'type'=>'control-panel-mail-alias','attributes'=>$item->only(['domain','address','destinations','active'])]],201);
    }

    public function controls(Request $request, ConfigureMailControls $configure): JsonResponse
    {
        $teamId=$request->user()?->current_team_id; abort_if($teamId===null,403,'A current team is required.'); $data=$request->validate(['mail_account_id'=>['required','uuid'],'spam_filter_enabled'=>['sometimes','boolean'],'spam_threshold'=>['nullable','integer','min:1','max:30'],'spam_action'=>['nullable','in:tag,quarantine,reject'],'virus_scan_enabled'=>['sometimes','boolean'],'autoresponder_enabled'=>['sometimes','boolean'],'autoresponder_subject'=>['nullable','string','max:255'],'autoresponder_message'=>['nullable','string','max:10000'],'keep_copy_on_server'=>['sometimes','boolean']]); $item=$configure->execute(array_merge($data,['team_id'=>$teamId])); return response()->json(['data'=>['id'=>$item->getKey(),'type'=>'control-panel-mail-control','attributes'=>$item->only(['mail_account_id','spam_filter_enabled','spam_threshold','spam_action','virus_scan_enabled','autoresponder_enabled','keep_copy_on_server'])]],201);
    }

    public function diagnostic(Request $request, RecordDeliveryDiagnostic $record): JsonResponse
    {
        $teamId=$request->user()?->current_team_id; abort_if($teamId===null,403,'A current team is required.'); $data=$request->validate(['mail_account_id'=>['nullable','uuid'],'message_id'=>['nullable','string','max:255'],'recipient'=>['required','email'],'status'=>['nullable','string','max:50'],'response'=>['nullable','string','max:10000']]); $item=$record->execute(array_merge($data,['team_id'=>$teamId])); return response()->json(['data'=>['id'=>$item->getKey(),'type'=>'control-panel-mail-delivery-diagnostic','attributes'=>$item->only(['mail_account_id','message_id','recipient','status','response','checked_at'])]],201);
    }
