...
final class OsPackageResource extends Resource
{
    ...
    public static function table(Table $table): Table { return $table->columns([TextColumn::make('name')->searchable(), TextColumn::make('version'), TextColumn::make('architecture'), TextColumn::make('status')->badge(), TextColumn::make('node_id')]); }
    public static function getEloquentQuery(): Builder { return parent::getEloquentQuery()->where('team_id', auth()->user()?->current_team_id); }
    public static function getPages(): array { return ['index' => ListOsPackages::route('/')]; }
}
