...
final class QuotaInventory extends Component
{
    ...
    public function save(SetFileQuota $set): void
    {
        ...
        $set->execute(['team_id' => (string) auth()->user()?->current_team_id, 'owner_id' => $this->ownerId !== '' ? $this->ownerId : null, 'limit_bytes' => $this->limitBytes]);
    ...
    {
        $quotas = FileQuota::query()->where('team_id', auth()->user()?->current_team_id)->latest()->paginate(min(max($this->perPage, 1), 100));
