...
final class CreateMailAlias
{
    public function execute(array $a): MailAlias
    {
        $address = trim((string) ($a['address'] ?? ''));
        $destinations = array_values(array_filter(array_map('trim', $a['destinations'] ?? [])));
        if (! filter_var($address.'@'.($a['domain'] ?? ''), FILTER_VALIDATE_EMAIL) || $destinations === []) {
